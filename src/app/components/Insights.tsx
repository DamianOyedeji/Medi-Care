...
const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: Array<{ payload: { label: string } }>; label?: string }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white/90 dark:bg-stone-800/90 backdrop-blur-sm p-3 rounded-xl shadow-lg border border-stone-100 dark:border-stone-700 text-sm">
        <p className="font-serif text-stone-800 dark:text-stone-100 font-medium mb-1">{dataPoint.label}</p>
        <p className="text-stone-500 dark:text-stone-400 text-xs">{label}</p>
      </div>
    );
  }
  return null;
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <header className="mb-8">
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold text-stone-800 dark:text-stone-100 mb-2 font-serif tracking-tight">Your Insights</h1>
            <p className="text-stone-500 dark:text-stone-400 font-medium">A gentle view of how you've been feeling over time.</p>
          </div>
        </header>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-stone-500 dark:text-stone-400 mb-4">No insights yet.</p>
            {moodEntryCount < REQUIRED_ENTRIES ? (
              <>
                ...
                <p className="text-stone-400 dark:text-stone-500 text-sm mb-6">
                  You currently have <span className="font-semibold text-stone-600 dark:text-stone-300">{moodEntryCount}</span> — just{' '}
                  <span className="font-semibold text-teal-600 dark:text-teal-400">{REQUIRED_ENTRIES - moodEntryCount} more</span> to go!
                </p>
          ...
          <>
            {moodData.length > 0 && (
              <motion.div
                ...
                className="bg-white dark:bg-stone-800 p-6 rounded-3xl shadow-sm border border-stone-100 dark:border-stone-700"
              >
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-lg font-serif font-semibold text-stone-700 dark:text-stone-200">Mood Trends</h2>
                  <span className="px-2 py-1 rounded-full bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-400 text-xs">Last 7 Days</span>
                </div>
                ...
                <div className="mt-4 flex justify-between text-xs text-stone-400 dark:text-stone-500 font-medium px-2">
            ...
            {insights.map((insight, i) => (
              <motion.div
                ...
                className="bg-teal-50/50 dark:bg-teal-900/20 p-6 rounded-3xl border border-teal-100/50 dark:border-teal-800/30 flex items-start gap-4"
              >
                <div className="bg-white dark:bg-stone-800 p-3 rounded-full text-teal-600 dark:text-teal-400 shadow-sm shrink-0">
                  <Sparkles size={20} />
                </div>
                <div>
                  <h3 className="font-serif font-semibold text-teal-900 dark:text-teal-100 mb-1">{insight.title}</h3>
                  <p className="text-teal-800/80 dark:text-teal-200/80 leading-relaxed">{insight.content}</p>
                </div>
              </motion.div>
            ))}
